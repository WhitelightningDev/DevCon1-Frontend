...
const SITE_URL = 'https://gordon-s-bay-squash-hub.vercel.app/'
const OUT_DIR = path.join(process.cwd(), 'public', 'work', 'gbsquash')
...
async function captureDesktop() {
  ...
  const target =
    (await page.getByRole('heading', { name: /book|court|challenge|league|ladder/i }).first().count()) > 0
      ? page.getByRole('heading', { name: /book|court|challenge|league|ladder/i }).first()
      : null
